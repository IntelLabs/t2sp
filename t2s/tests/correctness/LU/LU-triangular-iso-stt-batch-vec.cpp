...
int main(void) {
    // setenv("WAIT", "wait", 1);
    ImageParam A(type_of<TYPE>(), 4);

    // Macros: for convenient use.
    #define X                      bb, i,     j,     k,     b
    #define X_no_k                 bb, i,     j,            b
    #define X_k_minus_1            bb, i,     j,     k - 1, b
    #define X_j_minus_1            bb, i,     j - 1, k,     b
    #define X_i_minus_1            bb, i - 1, j,     k,     b
    ...
    PrevV(X)  = select(i >= k, select(k == 0, A(bb, i, j, b), V(X_k_minus_1)), 0);
    ...
    collector.vectorize(bb);
    unloader.vectorize(bb);
    unloader.isolate_consumer_chain(deserializer);
    deserializer.vectorize(bb);
    ...
    Buffer<TYPE> input_batch(BB, SIZE, SIZE, B);
    for (int b = 0; b < B; b++) {
        for (int j = 0; j < SIZE; j++) {
            for (int i = 0; i < SIZE; i++) {
                for (int bb = 0; bb < BB; bb++) {
                    input_batch(bb, i, j, b) = input(i, j);
                }
            }
        }
    }
    ...
    input_batch.set_host_dirty();

    A.set(input_batch);
    Target target = get_host_target();
    target.set_feature(Target::IntelFPGA);
    target.set_feature(Target::Debug);
    Buffer<TYPE> results = deserializer.realize({BB, SIZE, SIZE, B}, target);
...
void check(const Buffer<TYPE> &results, const Buffer<TYPE> &input) {
    ...
    for (int j = 0; j < SIZE; j++) {
      for (int i = 0; i < SIZE; i++) {
            bool correct = (abs(O(i, j) - results(0, i, j, 0)) < 1e-2);
            printf("%5.2f (%5.2f%s)", O(i, j), results(0, i, j, 0), correct ? "" : " !!");
            pass = pass && correct;
        }
        printf("\n");
    }
