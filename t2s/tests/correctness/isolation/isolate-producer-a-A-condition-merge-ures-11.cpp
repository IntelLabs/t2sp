...
int main(void) {
    ...
    Func A(PLACE0), B(PLACE0), C;
    Var i;
    B(i) = (i <= 2);
    A(i) = select(B(i), a(i) + 1, i * 2) * 2;
    C(i) = select(i <= 2, a(i) + 1, i * 2) * 2;
    ...
    a.set(in);
    Buffer<int> golden = C.realize(SIZE, target);
    ...
    std::cout << "Before isolation:\n"
              << Internal::to_string(B, true, true);

    B.isolate_producer_chain(a, A_feeder);
