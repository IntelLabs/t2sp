...
int main(void) {
    ...
    #define P_lastk_previousi      k + K - 1, j,     i - 1
...
#ifdef COMPLEX_MATRIX
    Xk.isolate_producer_chain({a_re, a_im}, ASerializer, ALoader, AFeeder);
#else
    ...
    RCollector.set_bounds(i, 0, I, j, 0, J);
...
#ifdef COMPLEX_MATRIX
    Buffer<float> resultQ_re(K, I);
    Buffer<float> resultQ_im(K, I);
    Buffer<float> resultR_re(J, I);
    Buffer<float> resultR_im(J, I);
...
#else
    Buffer<float> resultQ(K, I);
    Buffer<float> resultR(J, I);
...
void check(const Buffer<float> &resultQ, const Buffer<float> &resultR, const Buffer<float> &input) {
    ...
    for (int i = 1; i <= I; i++) {
        for (int j = 1; j <= J; j++) {
            if (j >= i) {
                bool correct = fabs(R(j, i) - resultR(j - 1, i - 1)) < 0.005;
                pass = pass && correct;
                printf("%6.2f (%6.2f%s)", R(j, i), resultR(j - 1, i - 1), correct ? "" : " !!");
            } else {
    ...
    for (int k = 0; k < K; k++) {
        for (int i = 1; i <= I; i++) {
            bool correct = fabs(Q(k, i) - resultQ(k, i - 1)) < 0.005;
            pass = pass && correct;
            printf("%6.2f (%6.2f%s)", Q(k, i), resultQ(k, i - 1), correct ? "" : " !!");
        }
        printf("\n");
    }
...
void check_complex(const Buffer<float> &resultQ_re, 
    ...
    for (int k = 0; k < K; k++) {
        for (int j = 0; j < J; j++) {
            ...
            bool correct = fabs(input_re(j, k) - golden_re) < 0.005 && fabs(input_im(j, k) - golden_im) < 0.005;
            pass = pass && correct;
            printf("(%6.2f, %6.2fi)%s[%6.2f, %6.2fi]   ", golden_re, golden_im, correct ? " " : " !", input_re(j, k), input_im(j, k));
        }
        printf("\n");
    }
    assert(pass);
