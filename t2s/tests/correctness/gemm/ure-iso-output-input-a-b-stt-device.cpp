...
int main(void) {
    ...
    Func feederA(PLACE1), feederB(PLACE1);
    firstk.isolate_producer_chain(a, feederA);
    firstk.isolate_producer_chain(b, feederB);

    Func consumer(PLACE1);
    c.isolate_consumer_chain(consumer);
    consumer.space_time_transform(jj, ii);
    ...
    Buffer<int> result = consumer.realize({JJ, II, JJJ, III, OJ, OI}, target);
