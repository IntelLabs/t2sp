...
int main(void) {
    ...
    C(P)       = select(firstk(P) == 0, 0, select(kkk == 0, C(P_ok_minus_1), C(P_kkk_minus_1))) + A(P) * B(P);
    }
    ...
    if (KK != OK) {
        ...
        firstk.merge_ures(lastk, A, B, C, c);
    }
    ...
    if (KK != OK) {
        ...
        firstk.isolate_producer_chain({a, fk, lk}, feederA);
    }
