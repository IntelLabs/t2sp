...
int main(void) {
    ...
    Func out2;
    X2(q2, iic2, ic2, oc2) = select(iic2 == 0 || q2 == Q-1,
                                    x(q2 + iic2 + ic2*IIC + oc2*IIC*IC),
                                    X2(q2+1, iic2-1, ic2, oc2));
    W2(q2, iic2, ic2, oc2) = select(iic2 == 0, w(q2),
    ...
    Func out;
    X(q, iic, ic, oc) = select(iic == 0 || q == Q-1,
                                x(q + iic + ic*IIC + oc*IIC*IC),
                                X(q+1, iic-1, ic, oc));
    W(q, iic, ic, oc) = select(iic == 0, w(q),
                                         W(q, iic-1, ic, oc));
