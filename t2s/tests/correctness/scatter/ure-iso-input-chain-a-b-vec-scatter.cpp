...
int main(void) {
    ...
    #define control Bool(), {P}
    #define compute Int(32), {P}

    Func firstk(control), firstkk(control), lastk(control); // Control UREs
    Func A(compute), B(compute), C(compute), c;             // Compute UREs
    ...
    Func feederA(Place::Device), feederB(Place::Device), loaderA(Place::Device), loaderB(Place::Device);
    firstk.isolate_producer_chain(a, feederA);
    firstk.isolate_producer_chain(b, feederB);
    feederA.scatter(a, ii);
    feederA.vectorize(kkk);
    feederB.scatter(b, jj);
