...
public:
    ...
    Stmt visit(const For *op) override {
        ...
        if (!no_pipelines && producing >= 0 && !f.has_extern_definition()) {
            ...
            debug(3) << "Box of " << stages[producing].name
                     << "computed by: " << op->name << "\n";
            for (size_t k = 0; k < box.size(); k++)
                debug(3) << "  " << box[k].min << " ... " << box[k].max << "\n";
        ...
        // The merged-ure (that rewrites realize node) requires re-define loop bounds.
        // When a matched loop is visited, it further restricts the loop bound.
        // For example, the LetStmt: let z.s0.oc.def.min = 0; let z.s0.oc.def.max = 3;
        // defines the original loop bound. However, when we enter the oc loop, it changes to:
        // let z.s0.oc.def.min = z.s0.oc; let z.s0.oc.def.max = z.s0.oc;
#if 0
        if (producing >= 0 && f.has_shift_reg()) {
            body = LetStmt::make(op->name + ".def.min", Variable::make(Int(32), op->name), body);
            body = LetStmt::make(op->name + ".def.max", Variable::make(Int(32), op->name), body);
        }
#endif
