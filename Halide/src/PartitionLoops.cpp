...
class FindSimplifications : public IRVisitor {
    ...
    void visit(const Select *op) override {
        ...
        bool has_false_branch = op->false_value.defined();
        bool likely_f = has_false_branch && has_uncaptured_likely_tag(op->false_value);

        if (!likely_t && !likely_f) {
            likely_t = has_likely_tag(op->true_value);
            likely_f = has_false_branch && has_likely_tag(op->false_value);
        }
