...
protected:
    class CodeGen_Clear_OpenCL_C: public IRPrinter {
    public:
        enum OutputKind {
            CHeader,
            CPlusPlusHeader,
        ...
        CodeGen_Clear_OpenCL_C(std::ostream &dest,
                               Target target,
                               OutputKind output_kind = CImplementation,
                               const std::string &include_guard = "");
        ~CodeGen_Clear_OpenCL_C() override;
    ...
    protected:
         ...
         */
        std::string print_type(Type, AppendSpaceIfNeeded space_option = DoNotAppendSpace);

        /** Emit a statement to reinterpret an expression as another type */
        std::string print_reinterpret(Type, Expr);
