...
void Module::compile(const std::map<Output, std::string> &output_files) const {
    ...
    if (contains(output_files, Output::host_header)) {
        ...
        std::ofstream file(output_files.at(Output::host_header));
        Internal::CodeGen_C cg(file,
                               target(),
                               Internal::CodeGen_C::HostHeader,
                               output_files.at(Output::host_header));
        cg.compile(*this);
    }
    if (contains(output_files, Output::host_src)) {
        debug(1) << "Module.compile(): host_src " << output_files.at(Output::host_src) << "\n";
        std::ofstream file(output_files.at(Output::host_src));
        Internal::CodeGen_C cg(file,
                               target(),
