...
CodeGen_Posix::Allocation CodeGen_Posix::create_allocation(const std::string &name, Type type, MemoryType memory_type,
    ...
    if (constant_bytes > 0) {
        int64_t requested_bytes = ((int64_t)constant_bytes) * type.bytes();
        if (requested_bytes > (static_cast<int64_t>(1) << 31) - 1) {
            user_error << requested_bytes << " bytes are requested to allocate for " << name
                       << ". The size exceeds 2^31 - 1.\n";
        }
        constant_bytes = static_cast<int32_t>(requested_bytes);
