...
/** Generate CM device code for Intel GPU, with the given function name.
 * Other parameters are set to default or omit. The kernel source code
 * filename is the same as fn_name */
void Pipeline::compile_to_cm(const string &fn_name,
                             const vector<Argument> &args,
                             const Target &target) {
    Module m = compile_to_module(args, fn_name, target);
